...
### Changed
- **Home page redesigned to a "Twin Cards" layout** (`KynEventPage.tsx`, `PhotoChallengeCard.tsx`) — fixes user-reported friction where Snap Hunt was pushed below the fold under a tall hero + descriptive paragraph. The tall hero is now a single compact header row (icon chip + event name + one-line dates/venue); Poll and Snap Hunt render as two equal-weight cards in a 2-column grid, both visible without scrolling on a phone screen. When no Snap Hunt challenge is active, the Poll card renders alone rather than stretching to fill the row. Three concepts (Twin Cards, Segmented Tabs, Icon Hub) were mocked up live and interactively for the user to compare before choosing Twin Cards — see the plan file for the mockup link. The long descriptive paragraph about the festival is dropped from this page (not moved elsewhere in this pass).
