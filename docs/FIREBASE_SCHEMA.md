...
`firestore.indexes.json` is currently empty on purpose: every query this app issues today is either a single-field equality filter, a single `orderBy`, or multiple independent equality filters on a small subcollection — none of which require an explicit composite index in current Firestore. If you add a query that combines `orderBy` with an inequality filter (e.g. "questions after a given order, only published"), Firestore will refuse it at runtime with a console link to the exact index to add — add it there and copy it into `firestore.indexes.json` so it's captured for the next `firebase deploy`.
