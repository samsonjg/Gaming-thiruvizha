...
- No realtime listeners (`onSnapshot`) are used anywhere yet — every read is a one-shot `getDocs`/`getDoc`. `[TBD]` if a future requirement needs live-updating admin views (e.g. a live results screen during an event), that would be the first place to introduce `onSnapshot`, scoped narrowly — but see the next point first, since it's a hard constraint on that decision.
- **Firestore uses the Lite SDK** (`firebase/firestore/lite`, not `firebase/firestore`) everywhere — every repository, `services/firebase/app.ts`, and every type import. This cut the Firestore chunk from ~154kB to ~51kB gzipped (roughly a 67% reduction) by dropping the realtime-sync/offline-persistence machinery (WebChannel, IndexedDB persistence) that this app has never used, in favor of `fetch`-based one-shot requests. The lite SDK supports everything this app actually calls — `collection`/`doc`/`getDoc`/`getDocs`/`addDoc`/`setDoc`/`updateDoc`/`deleteDoc`/`query`/`where`/`orderBy`/`limit`/`startAfter`/`writeBatch`/`increment` — but **does not support `onSnapshot` or `runTransaction`**. If a future requirement needs either of those, that specific call site (not the whole app) would need to import from `firebase/firestore` instead; don't switch the whole app back just to get one listener.
- `RankingQuestion` (the only question type using `@dnd-kit`, ~25-30kB gzipped) is `React.lazy`-loaded from `QuestionRenderer` — most polls never use a ranking question, so that dependency no longer ships in the eager bundle. Follow this pattern for any future question type with its own heavy dependency.
